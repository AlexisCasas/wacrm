...
export default function JoinPage() {
  ...
  const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
    ...
    server_error: { title: t('serverErrorTitle'), body: t('serverErrorBody') },
  };
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // account. The redeem RPC's error message is technical and
        // not localized, so it's logged rather than shown; the user
        // sees the stable, localized conflict copy instead.
        if (res.status === 409) {
          if (payload.error) console.error('[join] redeem 409:', payload.error);
          setConflictMessage(t('toastAlreadyInAccount'));
        } else {
          if (payload.error) console.error('[join] redeem failed:', payload.error);
          toast.error(t('toastFailedAccept'));
        }
        setAccepting(false);
        return;
      }
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('toastFailedSignOut'));
      setSigningOut(false);
    }
  }, [t]);
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t('acceptingMovesAccount', { accountName: peek.account_name })}
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('cantJoinTitle', { accountName: peek.account_name })}
              </DialogTitle>
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>{t('cantJoinBody', { accountName: peek.account_name })}</p>
            </div>
