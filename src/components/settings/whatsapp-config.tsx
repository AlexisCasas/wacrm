...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || t('saveConfigFailed'));
        setSaving(false);
        return;
      }
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          t('registerFailed', { reason: data.registration_error }),
          { duration: 12000 },
        );
        ...
        toast.success(
          data.phone_info?.verified_name
            ? t('liveConnectedNamed', { name: data.phone_info.verified_name })
            : t('connectedGeneric'),
        );
      ...
      if (payload.connected) {
        ...
        toast.success(
          payload.phone_info?.verified_name
            ? t('testConnectedNamed', { name: payload.phone_info.verified_name })
            : t('testConnectedGeneric')
        );
      } else {
        setConnectionStatus('disconnected');
        setResetReason(payload.needs_reset ? 'token_corrupted' : payload.reason === 'meta_api_error' ? 'meta_api_error' : null);
        setStatusMessage(payload.message || '');
        toast.error(payload.message || t('testFailedGeneric'));
      }
    } catch (err) {
      console.error('Test connection error:', err);
      setConnectionStatus('disconnected');
      toast.error(t('testFailedNetwork'));
    } finally {
      setTesting(false);
    }
  }
  ...
  async function handleReset() {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || t('resetConfigFailed'));
        return;
      }
      ...
      toast.error(t('resetConfigFailed'));
    } finally {
      setResetting(false);
    }
  }
